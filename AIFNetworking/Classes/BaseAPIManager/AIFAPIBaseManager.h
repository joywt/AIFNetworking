...
@class AIFAPIBaseManager;
// 在调用成功之后的params字典里面，用这个key可以取出requestID
static NSString * const kAIFAPIBaseManagerRequestID = @"kAIFAPIBaseManagerRequestID";
...
/*************************************************************************************************/
/*                              AIFAPIManagerApiCallBackDelegate                                  */
/*************************************************************************************************/
/**/
@protocol AIFAPIManagerApiCallBackDelegate <NSObject>
...
/*************************************************************************************************/
/*                               AIFAPIManagerCallbackDataReformer                                */
/*************************************************************************************************/
//负责重新组装API数据的对象
/*
 重组数据的场景：
 ...
 使用时思考 reformer 的需求方是哪个对象，需要进行判断才能灵活使用。比如一个 viewController 只有一个tableView 界面，reformer 此时需求方是 tableview ，需要返回一个 array 类型的 source。 但是这里真正复用的应该是 tableViewCell，那就没必要为了给 tableview 一个 reformer 属性而自定义一个 tableview。可直接在 viewController 生成。
 */

/*负责重新组装API数据的对象*/
@protocol AIFAPIManagerCallbackDataReformer  <NSObject>
...
/*
 ...
 代码样例：
 - (id)manager:(AIFAPIBaseManager *)manager reformData:(NSDictionary *)data
 {
...
//用于解析服务器返回的错误信息
@optional
- (id)manager:(AIFAPIBaseManager *)manager failedReform:(NSDictionary *)data;
@end


/*************************************************************************************************/
/*                                       AIFAPIManagerValidator                                   */
/*************************************************************************************************/
/*
 ...
 3.有一种情况是manager调用api时使用的参数不一定是明文传递的，有可能是从某个变量或者跨越了好多层的对象中来获得参数，那么在调用api的最后一关会有一个参数验证，当参数不对时不访问api，同时自身的errorType将会变为AIFAPIManagerErrorTypeParamsError。这个机制可以优化我们的app。
 */

@protocol AIFAPIManagerValidator <NSObject>

@required
/*
 manager在调用API之前可以验证这些参数，通过manager的回调函数告知上层controller。避免无效的API请求。加快响应速度，也可以多个manager共用.
 */
- (BOOL)manager:(AIFAPIBaseManager *)manager isCorrectWithParamsData:(id)data;
/*
 ...
 */
- (BOOL)manager:(AIFAPIBaseManager *)manager isCorrectWithCallBackData:(id)data;

@end


/*************************************************************************************************/
/*                              AIFAPIManagerParamSourceDelegate                                  */
/*************************************************************************************************/
/**/
@protocol AIFAPIManagerParamSourceDelegate <NSObject>
//让manager能够获取调用API所需要的数据
@required
- (id)paramsForApi:(AIFAPIBaseManager *)manager;

@end

/*************************************************************************************************/
/*                              AIFAPIManagerShowLoadingViewDelegate                                  */
/*************************************************************************************************/
@protocol AIFAPIManagerShowLoadingViewDelegate <NSObject>
 ...
 */
- (void)showLoadingView:(AIFAPIBaseManager *)manager;
@end



typedef NS_ENUM(NSInteger,AIFAPIManagerRequestType) {
    AIFAPIManagerRequestTypeGet,
    AIFAPIManagerRequestTypePost,
};
 ...
 */
typedef NS_ENUM (NSUInteger, AIFAPIManagerErrorType){
    AIFAPIManagerErrorTypeDefault,       //没有产生过API请求，这个是manager的默认状态。
    AIFAPIManagerErrorTypeSuccess,       //API请求成功且返回数据正确，此时manager的数据是可以直接拿来使用的。
    AIFAPIManagerErrorTypeNoContent,     //API请求成功但返回数据不正确。如果回调数据验证函数返回值为NO，manager的状态就会是这个。
    AIFAPIManagerErrorTypeParamsError,   //参数错误，此时manager不会调用API，因为参数验证是在调用API之前做的。
    AIFAPIManagerErrorTypeTimeout,       //请求超时。AIFApiProxy设置的是20秒超时，具体超时时间的设置请自己去看AIFApiProxy的相关代码。
    AIFAPIManagerErrorTypeNoNetWork      //网络不通。在调用API之前会判断一下当前网络是否通畅，这个也是在调用API之前验证的，和上面超时的状态是有区别的。
};
...
/*************************************************************************************************/
/*
 AIFAPIBaseManager的派生类必须符合这些protocal
 */

@protocol AIFAPIManager <NSObject>
@required
- (NSString *)methodName;
- (NSString *)moduleName;
- (NSString *)serviceType;
//- (NSString *)commonParamsGeneratorType;
- (AIFAPIManagerRequestType)requestType;
- (BOOL)shouldCache;
...
/*************************************************************************************************/
/*                                    AIFAPIManagerInterceptor                                    */
/*************************************************************************************************/
/*
 AIFAPIBaseManager的派生类必须符合这些protocal
 */
@protocol AIFAPIManagerInterceptor <NSObject>

@optional
- (BOOL)manager:(AIFAPIBaseManager *)manager beforePerformSuccessWithResponse:(AIFURLResponse *)response;
- (void)manager:(AIFAPIBaseManager *)manager afterPerformSuccessWithResponse:(AIFURLResponse *)response;

- (BOOL)manager:(AIFAPIBaseManager *)manager beforePerformFailWithResponse:(AIFURLResponse *)response;
- (void)manager:(AIFAPIBaseManager *)manager afterPerformFailWithResponse:(AIFURLResponse *)response;

- (BOOL)manager:(AIFAPIBaseManager *)manager shouldCallAPIWithParams:(id)params;
- (void)manager:(AIFAPIBaseManager *)manager afterCallingAPIWithParams:(id)params;
...
@interface AIFAPIBaseManager : NSObject
@property (nonatomic, weak) NSObject <AIFAPIManager> *child; //里面会调用到NSObject的方法，所以这里不用id
@property (nonatomic, weak) id <AIFAPIManagerParamSourceDelegate> paramSource;
@property (nonatomic, weak) id <AIFAPIManagerValidator> validator;
@property (nonatomic, weak) id <AIFAPIManagerApiCallBackDelegate> delegate;
@property (nonatomic, weak) id<AIFAPIManagerInterceptor> interceptor;
@property (nonatomic, weak) id<AIFAPIManagerShowLoadingViewDelegate> showLoadingView;
...
AIFAPIBaseManager会先调用这个函数，然后才会调用到 id<AIFAPIManagerValidator> 中的 manager:isCorrectWithParamsData:
